...
impl ContentCache {
    ...
    /// Publish provider-generated bytes under the same verified block keys used
    /// by ordinary reads. This is called before the corresponding derived node
    /// becomes visible, so a generated artifact cannot disappear with a small
    /// adapter scratch cache after its exact size and revision were announced.
    pub async fn stage(
        &self,
        scope: &Scope,
        node: &Node,
        ...
        if bytes.len() as u64 != node.size || node.content_revision().is_none() {
            return Err(ProviderError::Protocol(
                "staged content does not match its node",
            ));
        }
